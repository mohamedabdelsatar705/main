...
class UserSignupSerializer(serializers.Serializer):
    email = serializers.EmailField(max_length=100,allow_blank =True ,required=False,allow_null =True )
    mobile = serializers.DecimalField(max_digits=10, decimal_places=0,required= False,allow_null=False)  
    ...
    def create(self, validated_data):
        return UserSignup.objects.create(**validated_data)
    ...
    def validate(self, attrs):
        ...
        if UserSignup.objects.filter(email=email_).exists():
            if email_ != "": 
                raise serializers.ValidationError('Email is already in use')

        if UserSignup.objects.filter(mobile=mobile_).exists():
            if mobile_ != "":
...
class UserSerializer(serializers.Serializer):
    ...
    # mobile = serializers.CharField(max_length=11,required=True,min_length=11,allow_blank =False ,allow_null=False)
    mobile = serializers.DecimalField(max_digits=10, decimal_places=0,required= True,allow_null=False)  
    ...
    class Meta:
        model = User
        fields = [ "fullname","username","email","mobile","date_of_birth","gender","status","verified","type" ]
